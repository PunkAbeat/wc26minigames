...
**The site is LIVE at https://wc26minigames.com** (deployed 11 Jun, version 58b2d706; CF API token at ~/.config/cloudflare/wc26minigames_token, expiring post-tournament). Remaining for launch-complete: CF Web Analytics beacon token (owner, dashboard) → rebuild+redeploy; owner device pass of polish+feedback rounds; FIFA-wording copy pass; error monitoring; QA-listen (Iran/Paraguay/DR Congo); distribution push. Deploys: `npm run deploy` in app/ with CLOUDFLARE_API_TOKEN exported. Agent, if working before that lands: error monitoring (research/questions.md #4 needs the owner's pick first) or the FIFA-wording copy pass are the next unblocked items; the QA-listen list (Iran/Paraguay/DR Congo) needs human ears — the self-hosted copies in app/public/audio/ are the easiest way to listen.
