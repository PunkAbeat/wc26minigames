...
**Still open (the genuine owner call): product shape** — campaign ladder / play-your-nation / daily flag, or a mix (the grid already supports the first two). Picking it gates the real graduation: route + i18n + test suite + share grid, and an editorial accuracy pass on the flag specs (South Africa's pall and a few emblems are rough stylisations; some emblem colours are approximate). SHOOTOUT remains the standing alternative.
