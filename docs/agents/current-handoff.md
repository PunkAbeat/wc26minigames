...
**The site is LIVE at https://wc26minigames.com** (deployed 11 Jun, version 58b2d706; CF API token at ~/.config/cloudflare/wc26minigames_token, expiring post-tournament). Remaining for launch-complete: owner device pass of polish+feedback rounds; QA-listen (Iran/Paraguay/DR Congo); distribution push. Latest deploy: version 4fc0d6c6 (11 Jun, clean HEAD 3df3e40) — includes the FIFA-wording de-brand ([ADR-0006](../adr/0006-debranded-copy-nominative-use.md)), verified on prod. Deploys: `npm run deploy` in app/ with CLOUDFLARE_API_TOKEN exported. The FIFA-wording copy pass is **done** (full de-brand, owner decision 11 Jun — share text, titles/OG, manifest, kickers; disclaimer added to /anthem; factual puzzle hints kept). Error monitoring is done (Workers Logs + client beacon). The QA-listen list needs human ears — the self-hosted copies in app/public/audio/ are the easiest way to listen.
