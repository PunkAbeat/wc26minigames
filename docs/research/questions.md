...
6. **Game #2: GROUPS — launch decision.** ~~Selection~~ → owner picked **GROUPS** 12 Jun (full brainstorm + parked alternatives in [game2-brainstorm.md](game2-brainstorm.md)); prototype built same day on the tailnet preview (NOT deployed). Open: (a) does it graduate to production after the on-device pass? (b) puzzle-bank difficulty/quality verdict — 10 grids exist, ≥40 needed for the tournament window; (c) name — "GROUPS" puns on the group stage but check collisions before any launch.
