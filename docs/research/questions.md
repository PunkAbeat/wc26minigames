...
6. **Localization scope.** ~~Needs an owner call~~ → owner greenlit the phased plan 12 Jun; **phase 1 (EN+ES+FR) shipped same day** (UI chrome, share text, ES/FR nation names as guess aliases, lang switcher). Open remainder: phase-2 scope — per-nation hints/verdicts, canvas-card labels, og/meta per locale, and which languages come next (~30 participant languages total).
