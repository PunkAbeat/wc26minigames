...
- **Playback controller** (`pb` object): play/pause/resume toggle on the big button (▶/⏸ icons), real-time progress — a glowing playhead sweeps the track with a `▶ 3s / 7s` label (ball/fill remain *guess* progress; don't merge the two). Works for both real audio and the synth fallback (synth pause = stop + reschedule from offset). When the game ends (win or lose) the **full anthem auto-plays** as the reveal — only from the live finish (user-gesture chain); a restored finished game won't autoplay but the button replays the full anthem. Win confetti = centre fountain + side cannons + second wave (`confettiBurst`).
